 ...
 * 
FROM 
  health_tracker_data_2020 TABLESAMPLE (5 ROWS)
...
CREATE 
OR REPLACE TEMPORARY VIEW broken_readings
AS (
...
CREATE 
OR REPLACE TEMPORARY VIEW inserts AS (
  SELECT
    value.name,
    value.heartrate,
    CAST(FROM_UNIXTIME(value.time) AS timestamp) AS time,
...
CREATE 
OR REPLACE TEMPORARY VIEW upserts AS (
    SELECT * FROM updates 
    UNION ALL 
    SELECT * FROM inserts
    )
