...
-- Revisa si existe la tabla, la crea y carga datos desde parquet
...
-- Describe el tipo de variable -- info
DESCRIBE People10M;
...
-- Selecciona 4 variables de la tabla con las condiciones mayor a 1900 y genero femenino
SELECT
  firstName,
  middleName,
  lastName,
...
-- calcular el 20% de los ingresos por cada persona
SELECT
  firstName,
  lastName,
  salary,
...
-- crea una vista temporal y realiza los calculos anteriores
...
-- obtener el promedio de salario por año de nacimiento
SELECT
  birthYear,
...
-- Contar cuantos diferentes nombres existen
...
-- creara dos vistas con los nombres unicos
...
-- crear el join por medio del nombre
SELECT firstName 
...
-- contar cuantas filas tiene el join
SELECT count(*) 
FROM PeopleDistinctNames 
JOIN SSADistinctNames ON firstName = ssaFirstName;
